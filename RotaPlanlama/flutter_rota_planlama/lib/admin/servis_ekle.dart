...
class aracadd extends StatefulWidget {
  ...
  final String kul_id;
  final bool admin;
  aracadd(this.kul_id, this.admin);
  @override
  _aracaddState createState() => _aracaddState();
}

class _aracaddState extends State<aracadd> {
  TextEditingController plaka = new TextEditingController();
  var servis_kayit = {};
  ...
  Future<void> servisKayit() async {
    servis_kayit = {'plaka': plaka.text};
    var url = 'http://192.168.43.53:8000/admin/arac-add';
    print(servis_kayit);
    try {
      var response = await http.post(Uri.parse(url), body: servis_kayit);
